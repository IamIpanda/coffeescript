...
(function() {
  ...
  var BALANCED_PAIRS, CALL_CLOSERS, CONTROL_IN_IMPLICIT, DISCARDED, EXPLICIT_TYPE_ANNOTATIONS, EXPLICIT_TYPE_UNARY, EXPRESSION_CLOSE, EXPRESSION_END, EXPRESSION_START, IMPLICIT_CALL, IMPLICIT_END, IMPLICIT_FUNC, IMPLICIT_UNSPACED_CALL, INVERSES, LINEBREAKS, NOT_IMPLICIT_BEFORE, Rewriter, SINGLE_CLOSERS, SINGLE_LINERS, UNFINISHED, extractAllCommentTokens, generate, k, left, len, moveComments, right, throwSyntaxError,
  ...
  exports.Rewriter = Rewriter = (function() {
    class Rewriter {
      ...
      looksObjectish(j) {
        var end, index;
        if (this.indexOfTag(j, '@', null, ':') !== -1 || this.indexOfTag(j, null, '?', ':') !== -1 || this.indexOfTag(j, null, ':') !== -1) {
          return true;
        }
      ...
      addImplicitBracesAndParens() {
        ...
        return this.scanTokens(function(token, i, tokens) {
          var endImplicitCall, endImplicitObject, forward, implicitObjectContinues, implicitObjectIndent, inControlFlow, inExplicitType, inImplicit, inImplicitCall, inImplicitControl, inImplicitObject, isImplicit, isImplicitCall, isImplicitObject, k, newLine, nextTag, nextToken, offset, preContinuationLineIndent, preObjectToken, prevTag, prevToken, ref, ref1, ref2, ref3, ref4, s, sameLine, stackIdx, stackItem, stackTag, stackTop, startIdx, startImplicitCall, startImplicitObject, startIndex, startTag, startsLine, tag;
          [tag] = token;
          ...
          endImplicitObject = function(j) {
            ...
            i += 1;
            if (((ref = tokens[j + 1]) != null ? ref[0] : void 0) === 'OUTDENT' && ((ref1 = tokens[j + 2]) != null ? ref1[0] : void 0) === 'TERMINATOR' && ((ref2 = tokens[j + 3]) != null ? ref2[0] : void 0) === '=') { // assignment after possible type object
              tokens.splice(j + 2, 1);
              return i -= 1;
            }
          };
          ...
          inExplicitType = () => {
            return this.findTagsBackwards(i, EXPLICIT_TYPE_ANNOTATIONS);
          };
          ...
          if ((indexOf.call(IMPLICIT_FUNC, tag) >= 0 && token.spaced || tag === '?' && i > 0 && !tokens[i - 1].spaced) && (indexOf.call(IMPLICIT_CALL, nextTag) >= 0 || (nextTag === '...' && (ref = this.tag(i + 2), indexOf.call(IMPLICIT_CALL, ref) >= 0) && !this.findTagsBackwards(i, ['INDEX_START', '['])) || indexOf.call(IMPLICIT_UNSPACED_CALL, nextTag) >= 0 && !nextToken.spaced && !nextToken.newLine) && !inControlFlow()) {
            if (tag !== '?' && inExplicitType()) {
              if (ref1 = token[1], indexOf.call(EXPLICIT_TYPE_UNARY, ref1) >= 0) {
                // Forbid implicit calls within explicit type, and map keyof etc. to
                // appropriate token type for grammar.
                token[0] = 'EXPLICIT_TYPE_UNARY';
              }
            } else {
              if (tag === '?') {
          ...
          if (tag === ':') {
            ...
            s = (function() {
              ...
              switch (false) {
                case ref2 = this.tag(i - 1), indexOf.call(EXPRESSION_END, ref2) < 0:
                  ...
                  break;
                case this.tag(i - 1) !== '?':
                  return i - 2; // optional keys in object types
                case this.tag(i - 2) !== '@':
                  return i - 2;
                default:
                  return i - 1;
            ...
            if (stackTop()) {
              ...
              if ((stackTag === '{' || stackTag === 'INDENT' && this.tag(stackIdx - 1) === '{') && (startsLine || this.tag(s - 1) === ',' || this.tag(s - 1) === '{') && (ref3 = this.tag(s - 1), indexOf.call(UNFINISHED, ref3) < 0)) {
                return forward(1);
              }
            }
          ...
          if (tag === ',' && !this.looksObjectish(i + 1) && inImplicitObject() && !((ref4 = this.tag(i + 2)) === 'FOROF' || ref4 === 'FORIN') && (nextTag !== 'TERMINATOR' || !this.looksObjectish(i + 2))) {
      ...
      // blocks are added.
      normalizeLines() {
        var action, closeElseTag, condition, ifThens, inExplicitType, indent, leading_if_then, leading_switch_when, outdent, starter;
        ...
        condition = function(token, i) {
          ...
          return token[1] !== ';' && (ref = token[0], indexOf.call(SINGLE_CLOSERS, ref) >= 0) && !(token[0] === 'TERMINATOR' && (ref1 = this.tag(i + 1), indexOf.call(EXPRESSION_CLOSE, ref1) >= 0)) && !(token[0] === 'ELSE' && (starter !== 'THEN' || (leading_if_then || leading_switch_when))) && !(((ref2 = token[0]) === 'CATCH' || ref2 === 'FINALLY') && (starter === '->' || starter === '=>')) || (ref3 = token[0], indexOf.call(CALL_CLOSERS, ref3) >= 0) && (this.tokens[i - 1].newLine || this.tokens[i - 1][0] === 'OUTDENT') || inExplicitType && token[0] === '='; // switch from explicit type to assign
        };
        action = function(token, i) {
        ...
        return this.scanTokens(function(token, i, tokens) {
          ...
          if (indexOf.call(SINGLE_LINERS, tag) >= 0 && this.tag(i + 1) !== 'INDENT' && !(tag === 'ELSE' && this.tag(i + 1) === 'IF') && !conditionTag) {
            ...
            inExplicitType = this.findTagsBackwards(i, EXPLICIT_TYPE_ANNOTATIONS);
  ...
  exports.EXPLICIT_TYPE_ANNOTATIONS = EXPLICIT_TYPE_ANNOTATIONS = ['~', 'EXPLICIT_TYPE'];
