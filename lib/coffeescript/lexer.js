...
(function() {
  ...
  var {Rewriter, INVERSES, UNFINISHED, LINEBREAKS, EXPLICIT_TYPE_ANNOTATIONS} = require('./rewriter');
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    identifierToken() {
      ...
      var [input, id, colon, optional] = match;
      ...
      if (optional) {
        var optionalOffset = input.lastIndexOf('?');
        var optionalToken = this.token('?', '?', {
    ...
    jsxToken() {
      var end, jsxTag, notMatch, openingTagToken, prev, ref, tokens;
    ...
    literalToken() {
      var match, message, ref, ref1, ref2, ref3, ref4, ref5;
  ...
  var RESERVED = ['case', 'function', 'var', 'with', 'const', 'let', 'enum', 'native', 'implements', 'interface', 'package', 'private', 'protected', 'public', 'static'];
  ...
  var GENERIC_FUNCTION_NOT_JSX = /^<[^<>]*>\s*(?:\(([^<>]*))?[-=]>/; // generic parameters
  // nothing before arrow, or an open paren and no angles
  // arrow

  var NUMBER = /^0b[01](?:_?[01])*n?|^0o[0-7](?:_?[0-7])*n?|^0x[\da-f](?:_?[\da-f])*n?|^\d+n|^(?:\d(?:_?\d)*)?\.?(?:\d(?:_?\d)*)+(?:e[+-]?(?:\d(?:_?\d)*)+)?/i; // binary
  // octal
  // hex
  // decimal bigint
  // decimal
  ...
  var UNARY_MATH = ['!']; // '~', '-', '+' also sometimes act as UNARY_MATH

  var EXPLICIT_TYPE = [':=']; // '~' needs to be left alone, to allow for unary op
